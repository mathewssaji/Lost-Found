...
async def find_matches(
    ...
    # 4. Compute Composite Confidence Score for all candidates
    scored_matches = []
    for cand in candidates:
        cand_emb = cand.get("embedding", [])
        if "atlas_score" in cand:
            # Atlas provides cosine similarity directly in metadata
            vec_sim = float(cand["atlas_score"])
        else:
            vec_sim = cosine_similarity(query_vector, cand_emb)
        ...
        # Include candidate if it meets the minimum threshold or if we want top matches
